...
export default function ImageToPDFConverter() {
  ...
  // تابع برای بازسازی URLها در صورت نیاز - بهبود یافته
  const recreateImageUrls = useCallback(async () => {
    console.log("🔄 Recreating image URLs...");

    const imagesWithNewUrls = await Promise.all(
      images.map(async (img) => {
        // بررسی کنید که آیا URL هنوز معتبر است
        try {
          // اگر URL از بین رفته، جدید ایجاد کن
          const newUrl = URL.createObjectURL(img.file);
          console.log(`✅ Recreated URL for: ${img.name}`);
          return { ...img, url: newUrl };
        } catch (error) {
          console.error(`❌ Failed to recreate URL for: ${img.name}`, error);
          return img; // در صورت خطا، تصویر اصلی را برگردان
        }
      })
    );
  ...
  // در تابع handleConvert، بخش اعتبارسنجی را ساده‌تر کنید:
  const handleConvert = useCallback(async () => {
    ...
    try {
      console.log("🔄 Checking image URLs before conversion...");
      validImages = await recreateImageUrls();
    } catch (error) {
      console.warn("⚠️ Could not recreate URLs, using original images:", error);
    }

    // بررسی محدودیت‌های فنی
    const limitations = PDFConverter.checkTechnicalLimitations(validImages);
    ...
    // اعتبارسنجی تصاویر - با مدیریت خطای بهتر
    ...
    try {
      const { valid, invalid } = await PDFConverter.validateImages(validImages);
      ...
      console.log(`✅ Starting conversion of ${valid.length} valid images`);
  ...
  }, [images, settings, recreateImageUrls]);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-center p-4 max-h-[70vh] overflow-auto">
              <img
                ...
                onError={(e) => {
                  console.warn(
                    `❌ Preview image failed to load: ${selectedImage.name}`
                  );
                  // اگر تصویر لود نشد، URL جدید ایجاد کن
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-6xl mx-auto p-3 sm:p-4 md:p-6 backdrop-blur-sm">
      ...
      {/* Hidden file input - همیشه در دسترس */}
      <input
        ref={fileInputRef}
        type="file"
        multiple
